...
from .relative_transformer import RelativeMultiHeadAttn, RelativeSinusoidalPositionalEmbedding
...
class RelMultiHeadAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout, r_w_bias=None, r_r_bias=None, scale=False):
        ...
        self.dropout_layer = nn.Dropout(dropout)
        self.pos_embed1 = RelativeSinusoidalPositionalEmbedding(d_model//n_head, 0, 1200)
        self.pos_embed2 = RelativeSinusoidalPositionalEmbedding(d_model//n_head, 0, 1200)
    ...
    def forward(self, x, mask):
        batch_size, max_len, d_model = x.size()
        [mask1, mask2] = torch.chunk(mask,2,1)
        pos_embed1 = self.pos_embed1(mask1)
        pos_embed2 = self.pos_embed2(mask2)
